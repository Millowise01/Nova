...
| F4 E2E workflow                  | **Done**                                        | The backend starts on Node 20; three browser suites (web, seller, admin: 30 tests) pass locally and on GitHub. See item F4.                               |
...
**F4. E2E has never passed in CI (0 of 15 runs before this work): done (2026-09-21).**

- _Root cause_ **[V, R]**: `nest build` compiles the backend to CommonJS, and `dist/config/env.schema.js` does `require("@nova/validation")`. That package's entry is raw TypeScript ESM (`main: ./src/index.ts`, `"type": "module"`). Jest maps it to source and Node 24 strips types natively, so it worked locally. On Node 20.20.2 (CI's, and inside the repository's declared `>=20.18`), Node tried to run the `.ts` file as JavaScript and died with `SyntaxError: Unexpected token 'export'`. Reproduced locally on Node 20.20.2 with the same trace as GitHub's; the same load succeeds on Node 24. It was not the Playwright configuration, the workflow's Node setup, Prisma, the environment variables or the services: the checklist was walked in order and the trace names the file. **The built backend could not start on the Node range the repository declares**, so this would also have hit any deployment.
- _Fix:_ `@nova/validation` also builds `dist/cjs` and declares `exports` (`require` to the compiled copy; `types` and `default` still to the source), so Next, Vitest, `tsc` and Jest resolve exactly as before. This adds a build output for the one consumer that runs uncompiled; it does not convert the package between module systems. Recorded as ADR-0004. Rejected: running E2E on Node 24 (it would hide the defect and lean on Node's type stripping, which the declared minimum lacks).
- _What else an empty CI database needed, found by running the suites against a fresh database, not by guessing:_ the specs relied on data left in a developer's database ("real published products", categories from earlier test runs). `backend/scripts/e2e-seed.mjs` now creates one category and one published product through the real API, and `e2e-accounts.mjs` creates the throwaway accounts (role promotion stays the one test-only Prisma step, as in `promote-role.ts`; passwords are generated per run and never stored).
- _Workflow:_ `.github/actions/e2e-backend` builds the backend and its workspace dependencies (`pnpm --filter "@nova/backend..." build`), starts it, waits until it answers and, if it exits or times out, prints its log and fails, instead of the silent `wait-on` timeout that hid this for 15 runs. It seeds the catalog. The three jobs share it and `backend-test-env`, and upload the backend log. Job IDs are unchanged so branch protection keeps matching; an `admin-auth` job and `apps/admin` Playwright setup are new.
- _Two more defects in the existing web purchase journey, found once the backend ran:_ each product card now has two links (the image, whose text is "No image", and the title), so `.first()` took "No image" as the title (a drift from the design-system conformance commit that nothing caught because E2E never ran); and the phone number came from the leading digits of `Date.now()`, which repeat for about 100 seconds, so back-to-back runs got a 409 (a failure I reproduced, then confirmed against the backend log, before changing it). Both fixed without weakening an assertion.
- _Two real product defects, found by the new browser specs and each reproduced first_ **[V, R]**: (1) after a reload, a page that fetches on mount got a 401 and refreshed while `AuthProvider`'s boot-time restore refreshed too; the refresh token is single-use, so the second call was rejected (two concurrent `POST /auth/refresh` in the backend log, 200 and 401) and the client cleared its tokens, leaving an "We couldn't load your order history" page. (2) Clicking "Sign out" before that restore returned was undone: it stored fresh tokens and signed the user back in (2 of 3 browser runs failed before, 3 of 3 passed after). Both were in the three original per-app providers too. Fixed in `@nova/api-client` (the single-flight refresh is exported and the restore goes through it) and `AuthProvider`, each test-first, in their own commit so they can be reviewed apart from the E2E work. This is auth code changed because a real defect was demonstrated, not to make a test pass.
- _Local results_ (Chromium via Playwright, Desktop Chrome profile; backend started with `node dist/main.js` on **Node 20.20.2** against a freshly migrated, empty database, seeded by the script; each app a production build served by `next start`):

| Suite                             | Tests | Passed | Failed | Skipped | Runs                                        |
| --------------------------------- | ----- | ------ | ------ | ------- | ------------------------------------------- |
| `web` (purchase journey + 4 auth) | 5     | 5      | 0      | 0       | 3 back to back                              |
| `seller` (onboarding + 12 auth)   | 13    | 13     | 0      | 0       | 3 back to back, then 2 more after a rebuild |
| `admin` (12 auth)                 | 12    | 12     | 0      | 0       | 2                                           |

The auth specs cover sign-in, reload and session restoration, sign-out (seller and admin; `web` has no sign-out control yet), a garbage cookie, a forged cookie claiming the right role (the app drops it and the API answers 401), the wrong role (`/forbidden`), the right role opening a protected page directly, `/login-history` staying protected, and five hostile redirect forms.

- _Redirect sanitization, in a browser:_ with the original `sanitizeRedirect` temporarily restored and `seller` rebuilt, the backslash, tab and newline cases fail (the app leaves the origin) and the other two pass; with the shared one all five pass. So the browser test can detect the weakness from step C, and the fix closes it.
- _GitHub:_ GitHub ran the E2E workflow on `3de3003`: `purchase-journey` 5 of 5, `seller-onboarding` 13 of 13 and `admin-auth` 12 of 12 passed, so **30 of 30, none failed, skipped or needing a retry** (CI allows two), in Chromium, with the backend answering on **Node v20.20.2** and the seed creating its category and product. `verify` passed as well, and one route (`web` /wallet) grew by 1 kB, nothing else changed. The first push of this work (`6851a58`) failed for two reasons found and fixed: `@nova/admin#test` ran the new Playwright specs under Vitest (admin's config lacked the `e2e/**` exclusion that seller and web have, and my local admin unit-test run predated the e2e folder), and all three app builds failed with `<Html> should not be imported outside of pages/_document` because the jobs set `NODE_ENV=development` for the whole job for the backend's sake (reproduced locally; it is now set on the backend start step only). On that first push the backend build, start-up and seed steps already passed in every job.
- _CI path selection_ (`select-scope.sh`): exercised with the real script and real `turbo` in throwaway worktrees at real commits. `all` for a new branch (base unavailable) and for a diff touching CI or workspace configuration; `filtered` for an apps-only diff (`@nova/admin @nova/seller @nova/web`) and for a pull-request code commit (`@nova/admin @nova/eslint-config @nova/seller`); `none` for a docs-only diff on both `push` and `pull_request`, with the summary saying no lint, typecheck, test or build ran. The workflow gates lint, typecheck, the backend environment, test and build on `mode != 'none'`; secretlint, semgrep and `format:check` always run. **Not yet seen on GitHub:** `filtered` and `none`, because this branch's diff against `main` always contains CI files, so its runs are `all`. Seeing them there needs two throwaway pull requests (a docs-only one and a one-package one); they were not opened without your go-ahead. The E2E workflow has its own `paths` filter, so a docs-only pull request does not start it.
- _Not done here:_ the session cookie hardening (see S1), F3, F6 and F10, and a sign-out control for `web`.
...
**S1. Session cookie hardening: scheduled security task, not started (explicitly out of F4; production cookie behaviour was not changed).** Today the session summary (`userId`, `roles`, `expiresAt`) is written by the app's own JavaScript with `path=/; max-age=86400; SameSite=Lax`. The middleware and the server-rendered layout read it; the backend never does. To cover: **`Secure`** on HTTPS; **`SameSite`** (`Lax` today; decide `Strict` for the seller and admin apps against the sign-in-from-link flows); **`HttpOnly` where appropriate** (it cannot be set from `document.cookie`, so it means the server, or a Next route handler, sets the cookie, which also makes it un-forgeable from script but not un-forgeable by the user); **path and domain** (`localhost` cookies are not port-isolated, which is why each app has its own name; production needs a domain decision so `web`, `seller` and `admin` cannot read each other's); **expiration** (24 hours against a short-lived access token, and no server-side revocation: the backend has no logout endpoint); the **script-readable session summary** (it exists so the client and the layout can render without a request; the alternative is to derive it from the server); and the **SSR and client implications** (`initialSession` is parsed from the cookie in the root layout, so anything that moves the cookie server-side changes hydration and the boot-time restore). Related observations, not part of this task: the refresh token lives in `localStorage` (readable by script), and the guard is a navigation aid, so the backend must keep being the only authorization boundary. Needs a design decision from the repository owner before any implementation.

...
- Peach is for **soft backgrounds** only — never for text or borders.
...
bg-primary             Nova Navy — primary actions
bg-accent              Nova Orange — CTAs, promotions
...
<button className="bg-[#002A63] text-white">Buy Now</button>
...
<p className="text-base text-foreground-muted leading-relaxed">Body copy</p>
<span className="text-xs font-semibold uppercase tracking-widest text-foreground-subtle">Category</span>
```
...
3. **No hardcoded brand colors** — `#002A63`, `#FF6A00`, etc. must never appear in component code. Use `bg-primary`, `bg-accent`, or `cssVars.color.primary`.
4. **No arbitrary Tailwind values** — `bg-[#002A63]` is forbidden. If a value isn't in the token system, add it to the token system first.

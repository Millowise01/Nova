...
- **Phase 1 status:** **Refunds: implemented** (`backend/src/modules/payments-wallet/domain/refunds.service.ts` — `REFUND_DUAL_AUTH_THRESHOLD`, propose/approve, self-approval blocked). **Payouts, role assignment changes, seller approval/suspension, direct ledger adjustments: still not applicable — re-confirmed, not just carried forward.** None of these exist as callable operations anywhere in `backend/src` (checked directly, not assumed): there is no payout endpoint (Finance, still Phase 2 remaining), no role-assignment endpoint (`test-utils/promote-role.ts` sets roles directly via Prisma for tests only, bypassing the API on purpose — there is no production path that does this), no seller suspension endpoint, and no ledger-adjustment endpoint beyond what refunds already write through `WalletService`. Building an approval workflow around operations that don't exist would be authorization theater, not a real control — extending this pattern to each one is correct future work the moment Finance/Trust & Safety actually add the corresponding endpoint, not before.
...
- **Phase 1 status:** **Implemented and verified.** 15-minute access tokens, single-use rotating refresh tokens with a server-side `Session` table enabling revocation, now signed **RS256** (asymmetric — migrated off HS256 this pass, see `backend/docs/05-security-baseline.md`).

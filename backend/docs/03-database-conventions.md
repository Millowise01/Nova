...
| Redis                 | Cache, rate-limit counters, transient session state (including OTP codes — see `backend/src/modules/identity/domain/otp.service.ts`, TTL-expired and single-use via `DEL`, not a Postgres table), distributed locks, internal pub/sub. Not a system of record for anything. |

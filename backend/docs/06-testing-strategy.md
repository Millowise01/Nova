...
- **Signup conflict from colliding test phone numbers: observed once, frequency unknown.** Several helpers build a phone number from a fixed prefix plus four hex characters (65,536 values) or from six random digits (900,000). The database is never cleaned, so users accumulate, and a new signup can hit an existing user's unique `phoneHash`. One instrumented run captured exactly that: `409 PHONE_ALREADY_REGISTERED` inside a helper, which then failed with `Cannot read properties of undefined`. That explains that failure; it is not shown to explain every "seller.body.data is undefined" failure, including one seen on an empty database in CI.

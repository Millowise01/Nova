...
Two intermittent failures are known. Neither is fixed, and neither has been reproduced on demand:

- **Signup conflict from colliding test phone numbers (suspected).** Several helpers build a phone number from a fixed prefix plus four hex characters (65,536 values), for example `+2327600${suffix.slice(0, 4)}`. A new signup can collide with an accumulated user's unique `phoneHash` and get a 409, which breaks the helper (`seller.body.data` is undefined). Evidence: one failing full run logged three signup 409s where the specs deliberately produce two, and the failure was inside a helper's signup. That run's log was not kept, so this is inference. To confirm, log the status and error code when a helper signup fails and run the orders spec repeatedly.
- **Notifications outbox-drain race (suspected).** The outbox relay claims rows with `FOR UPDATE SKIP LOCKED`, and the app also runs its own background relay tick. If that tick has an event locked in flight, a test's `drainOutbox()` sees nothing pending and returns while the handler is still writing, so a following read can miss the result. `notifications.integration.spec.ts` has failed this way at least twice under load. The fix belongs in the test helper (wait until no claimed rows remain) or in disabling the background tick for test apps.

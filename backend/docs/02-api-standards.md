...
## OpenAPI document — known gap, logged not fixed

> **Real, disclosed gap — `backend/openapi.json` has no response schemas.** `buildOpenApiDocument()` (`backend/src/main.ts`) calls `mergeZodRequestBodies()` (`backend/src/openapi/zod-openapi.ts`) to backfill request bodies from the same Zod schemas `ZodValidationPipe` validates against, but there is no equivalent `mergeZodResponses()` — every operation's `200`/`201` response in the generated spec is just `{"description": ""}`, with no schema at all (confirmed directly: `GET /v1/products`'s `responses` object has no `content`/`schema` key). This wasn't a gap in practice for `apps/web`, because `@nova/api-client` never reads the spec for response types — it derives them from the same `@nova/validation` Zod schemas directly via `z.infer<>`, and validates every response against them at runtime (see [[backend-implementation-state]] equivalent reasoning, or `packages/api-client/src/endpoints/*.ts`). It matters for anything that treats `openapi.json` itself as the contract — a future external consumer, an OpenAPI-codegen-based client, or Swagger UI at `/docs`, all of which currently show accurate requests but an undocumented response shape for every endpoint. **Future work:** add a `mergeZodResponses()` alongside the existing request-body merge, keyed off the same per-route Zod schema already used to `.parse()` each response in `@nova/api-client`. Not attempted in the apps/web wiring pass that found this (2026-09-16) — it's backend work, out of scope for a frontend-only pass.

      ...
      <div>
        ...
        <div className="grid grid-cols-2 gap-3">
          {(["customer", "seller"] as const).map((r) => (
            <label key={r} className={`flex cursor-pointer items-center gap-3 rounded-xl border p-4 transition ${role === r ? "border-primary bg-primary/5 dark:bg-primary/10" : "border-slate-200 dark:border-slate-700"}`}>
              <input type="radio" value={r} {...register("role")} className="accent-primary" />
              <div>
                <p className="text-sm font-semibold">{r === "customer" ? "Shop" : "Sell"}</p>
                <p className="text-xs text-slate-500">{r === "customer" ? "Buy products" : "List & sell"}</p>
              </div>
            </label>
          ))}
        </div>
      </div>
      <Button type="submit" disabled={isSubmitting} className="w-full py-3">{isSubmitting ? "Creating account…" : "Create Account"}</Button>

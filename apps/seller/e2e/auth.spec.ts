...
/**
 * Browser-level checks of the seller portal's authentication and navigation guard: sign-in, session
 ...
 */
const SESSION_COOKIE = "nova_seller_session";
const REFRESH_TOKEN_KEY = "nova_refresh_token";
const PROTECTED_PATH = "/kyc";
const FORBIDDEN_HEADING = "You don't have access to Nova Seller";
const API_BASE_URL = process.env.NEXT_PUBLIC_API_BASE_URL ?? "http://localhost:4000/v1";

function credentials(kind: "staff" | "customer") {
  const email = kind === "staff" ? process.env.SELLER_E2E_EMAIL : process.env.E2E_CUSTOMER_EMAIL;
  const password =
    kind === "staff" ? process.env.SELLER_E2E_PASSWORD : process.env.E2E_CUSTOMER_PASSWORD;
...
test.describe("the navigation guard", () => {
  ...
  test("a signed-in user without the seller role is sent to the forbidden page", async ({
    page,
  }) => {
  ...
  test("a seller can open a protected page directly", async ({ page }) => {
    await signIn(page);
    await expect(overview(page)).toBeVisible({ timeout: 15_000 });

    await page.goto(PROTECTED_PATH);
    await expect(page).toHaveURL(new RegExp(`${PROTECTED_PATH}$`));
    await expect(page.getByRole("link", { name: "Verification (KYC)" })).toBeVisible();
  });

 ...
 *  token just came straight from a trusted response body (signup/login/refresh);
 *  this is never used to authorize anything, only to read `sub`/`roles`/`exp` for
 *  the lightweight client-side session summary. Real verification is the
 *  backend's job on every subsequent request. */
